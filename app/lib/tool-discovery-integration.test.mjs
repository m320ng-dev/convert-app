...
const missingPreviouslyToolIds = [
  'html-to-markdown',
  'js-beautifier',
  'image-to-base64',
  'base64-to-image',
...
test('신규 도구 카드 텍스트는 제목, 설명, 메타 정보 역할을 분리한다', () => {
  ...
  assert.match(homeSource, /converter\.localOnly \? '브라우저 로컬' : '서버\/API 사용'/);
  assert.match(homeSource, /converter\.hasCopyButton \? '복사 지원' : '다운로드 지원'/);
...
test('신규 도구 카드는 기존 카드 그리드의 반응형 열 규칙 안에 렌더링된다', () => {
  ...
  assert.ok(tools.length > browserLocalToolCatalog.length);
...
test('신규 도구 카드는 기존 도구 카드의 아이콘 배지와 액션 영역 구조를 재사용한다', () => {
  ...
  assert.match(homeSource, /<BadgeIcon size=\{16\} \/>/);
  assert.match(
    homeSource,
...
test('신규 도구 목록 정렬은 전체 converter 등록 순서와 사용 순위를 일치시킨다', () => {
...
test('기존 실행 화면 도구도 첫 페이지 카드와 내비게이션에 노출된다', () => {
  for (const toolId of missingPreviouslyToolIds) {
    const listedTool = tools.find((tool) => tool.id === toolId);
    const navigationItem = getToolNavigationItems().find((item) => item.id === toolId);

    assert.ok(listedTool, `${toolId} 도구 목록 항목이 있어야 한다`);
    assert.equal(listedTool.href, `/converters/${toolId}`);
