...
import {
  convertYamlJson,
  type YamlJsonConversionMode,
} from '@/app/lib/yaml-json-converter';
import {
...
const modeOptions: Array<{
...
}> = [
  {
    id: 'yaml-to-json',
    label: 'YAML → JSON',
    description: '간단한 YAML 객체와 배열을 정렬된 JSON으로 변환합니다.',
  },
  {
    id: 'json-to-yaml',
    label: 'JSON → YAML',
    description: 'JSON 값을 들여쓰기 기반 YAML 텍스트로 변환합니다.',
  },
];

const sampleInputs: Record<YamlJsonConversionMode, string> = {
  'yaml-to-json': 'name: convertapp\nlocalOnly: true\ntags:\n  - api\n  - utility',
  'json-to-yaml': '{"name":"convertapp","localOnly":true,"tags":["api","utility"]}',
};

const emptyInputFailure = createInputValidationFailure('YAML 또는 JSON을 입력해주세요.');

export default function YamlJsonConverterPage() {
  const [mode, setMode] = useState<YamlJsonConversionMode>('yaml-to-json');
  const [input, setInput] = useState(sampleInputs['yaml-to-json']);

  function handleModeChange(nextMode: YamlJsonConversionMode) {
    setMode(nextMode);
  ...
  const { output, error, emptyMessage } = useMemo(() => {
    ...
    return executeToolConversion({
      input,
      transform: (input) => convertYamlJson(input, mode),
      emptyInputMessage: emptyInputFailure.message,
      emptyResultMessage: 'YAML 또는 JSON을 입력하면 결과가 표시됩니다.',
      defaultErrorMessage: 'YAML/JSON 변환 중 오류가 발생했습니다.',
    });
  }, [input, mode]);
    ...
    <div className="app-stack">
      <section className="app-panel app-panel-flat app-panel-body">
        ...
        <div className="mt-5 grid gap-4">
          <div>
            <label htmlFor="yaml-json-converter-mode" className="text-sm font-semibold text-slate-800">
              변환 모드
            </label>
            <select
              id="yaml-json-converter-mode"
              value={mode}
              onChange={(event) => handleModeChange(event.target.value as YamlJsonConversionMode)}
          ...
          <TextToolInput
            ...
            label={mode === 'yaml-to-json' ? 'YAML 입력' : 'JSON 입력'}
            value={input}
            onValueChange={setInput}
            exampleValue={sampleInputs[mode]}
            placeholder={mode === 'yaml-to-json' ? 'name: convertapp\nlocalOnly: true' : '{"name":"convertapp"}'}
      ...
      <ResultsPanel
        title="변환 결과"
        description="선택한 방향에 맞춰 YAML과 JSON을 즉시 변환합니다."
        copyValue={copyValue}
        copyLabel="결과 복사"
        copyAriaLabel="YAML/JSON 변환 결과 복사"
        copyCopiedMessage="YAML/JSON 변환 결과를 클립보드에 복사했습니다."
        copyEmptyMessage="복사할 YAML/JSON 변환 결과가 없습니다."
